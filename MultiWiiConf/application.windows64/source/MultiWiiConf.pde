...
void draw() {
  ...
  if (init_com == 1 && graph_on == 1) {
        ...
        if (toggleMotor) {
          ...
          for (i = 0; i < 8; i++) {
            if (motToggle[i].getState()) {
              payload.add(char(int(motorControlSlider.getValue()) % 256));
              payload.add(char(int(motorControlSlider.getValue()) / 256));
            } else {
              payload.add(char(1000 % 256));
              payload.add(char(1000 / 256));
            }
          }
    ...
    if ((toggleWriteServo || toggleWriteServoLive) && !toggleRead) { // MSP_SET_SERVO_CONF
      ...
      if (multiType == AIRPLANE || multiType == PPM_TO_SERVO || multiType == HELI_120_CCPM || multiType == HELI_90_DEG) {
        for (i = 0; i < 8; i++) {
          ...
          payload.add(char(q % 256));
          payload.add(char(q / 256)); // Min
          q = (int) ServoSliderMAX[i].value();
          payload.add(char(q % 256));
          payload.add(char(q / 256)); // Max
          q = (int)(ServoSliderC[i].value());
          payload.add(char(q % 256));
          payload.add(char(q / 256)); // Servo centers
    ...
    if (toggleWrite) {
      ...
      payload.add(char(intPowerTrigger / 256)); //a
      ...
      payload.add(char(nn >> 8)); // f


      nn = round(confINF[5].value());
      payload.add(char(nn - ((nn >> 8) << 8)));
      payload.add(char(nn >> 8)); // g 32b
      payload.add(char(nn - ((nn >> 16) << 16)));
      payload.add(char(nn >> 16));


      // MagDec
      nn = round(confINF[6].value() * 10);
      payload.add(char(nn - ((nn >> 8) << 8)));
      payload.add(char(nn >> 8)); // h


      // VBatscale
      nn = round(VBat[0].value());
      payload.add(char(nn)); // i
      for (i = 1; i < 4; i++) {
        int q = int(VBat[i].value() * 10);
        payload.add(char(q));
      } // j,k,l
  ...
  if (toggleGimbal || toggleTrigger) { //if (multiType == GIMBAL)
    ...
    line(0, 0, 1.8 * size, size);
    line(1.8 * size, size, 1.8 * size, size - 30);
    line(1.8 * size, size - 30, 0, -1.5 * size);
    line(0, 0, -1.8 * size, +size);
    line(-1.8 * size, size, -1.8 * size, +size - 30);
    line(-1.8 * size, size - 30, 0, -1.5 * size);
    noLights();
    ...
    line(+size, +1.2 * size, +size, -1.2 * size);
    line(-size, +1.2 * size, -size, -1.2 * size);
    line(+size, 0, 0, 0);
    line(-size, 0, 0, 0);
    noLights();
    ...
    line(-Span, VingRoot, 0, VingRoot);
    // Stab
    line(-(size * 0.4), size, (size * 0.4), size);
    line(-(size * 0.4), size + 5, (size * 0.4), size + 5);
    line(-(size * 0.4), size, -(size * 0.4), size + 5);
    line((size * 0.4), size, (size * 0.4), size + 5);
    // Body  
    ...
    // Body  
    line(0, 1.5 * scalesize, -5, -2, -0.5 * scalesize, -5);
    line(0, 1.5 * scalesize, -5, 2, -0.5 * scalesize, -5);
    line(-2, -0.5 * scalesize, -5, 2, -0.5 * scalesize, -5);
    // Fin 
    ...
    // Stab
    line(-(scalesize * 0.3), scalesize, -5, (scalesize * 0.3), scalesize, -5);
    line(-(scalesize * 0.3), scalesize + 3, -5, (scalesize * 0.3), scalesize + 3, -5);
    line(-(scalesize * 0.3), scalesize, -5, -(scalesize * 0.3), scalesize + 3, -5);
    line((scalesize * 0.3), scalesize, -5, (scalesize * 0.3), scalesize + 3, -5);
    ...
    ellipse(0, 0, 2 * scalesize, 2 * scalesize);
    // Body  
    line(0, 1.5 * scalesize, -2, -0.5 * scalesize);
    line(0, 1.5 * scalesize, 2, -0.5 * scalesize);
    line(-2, -0.5 * scalesize, 2, -0.5 * scalesize);
    // Fin 
    float finpos = scalesize * 1.3;
    int HFin = 5;
    int LFin = 10;
    ...
    // Stab
    line(-(scalesize * 0.3), scalesize, (scalesize * 0.3), scalesize);
    line(-(scalesize * 0.3), scalesize + 3, (scalesize * 0.3), scalesize + 3);
    line(-(scalesize * 0.3), scalesize, -(scalesize * 0.3), scalesize + 3);
    line((scalesize * 0.3), scalesize, (scalesize * 0.3), scalesize + 3);
